...
- **不抢已有服务**:探测到 3080 端口已被占用时,只开浏览器、不重复启动、不杀原服务
- **单实例**:重复双击不会重复启动
...
| 场景 | 程序行为 |
|---|---|
| 3080 端口空闲 | 静默启动 `npx --yes @deepseek-ai/dsh web --no-open` → 等就绪 → 打开全新 Chrome 窗口 |
| 3080 已被其它 dsh web 占用 | 不重复启动,直接打开浏览器页面;关窗后程序退出,**不影响**原有服务 |
| 双击时已有实例在运行 | 单实例锁生效,第二个实例直接退出 |
| 用户关闭打开的 Chrome 窗口 | 结束 dsh web 进程树 → 清理临时配置 → 程序退出(服务由它启动时才停) |
