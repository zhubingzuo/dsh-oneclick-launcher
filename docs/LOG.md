...
- 2026-09-12 | (回退) | **按要求回退"地址栏安装图标"相关的全部改动**:1.3.0 的 `seed_chrome_prefs`/`block_web_app_install`、1.4.x 的 `window_mode`(应用窗口)及其文档说明全部移除,代码内容与 1.2.0(`3b8ba48`)一致;仅保留 1.4.1 中与图标无关的那条"启动器已在运行时弹中文提示"。地址栏里的"安装"图标按用户要求保留(Chrome 对可安装网页的正常提示)。为避开已占用的 `v1.2.0` 标签,版本号记为 **1.5.0**;回归 32/32 PASS
