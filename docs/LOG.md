...
- 2026-09-12 | (本次续) | 用户反馈"地址栏总显示安装图标":根因是 DSH 前端自带 `manifest.webmanifest` 使页面可安装。新增 `seed_chrome_prefs`,在 Chrome 启动前把临时 profile 的 `default_content_setting_values.web_app_installation` 置为阻止(= Chrome 设置里的"Web 应用安装 → 不允许"),配置项 `block_web_app_install` 可关闭;已用 headless Chrome 实测该键被保留;版本 1.3.0
